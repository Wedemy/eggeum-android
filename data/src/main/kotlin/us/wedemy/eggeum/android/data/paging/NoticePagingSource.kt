...
public class NoticePagingSource(
...
) : PagingSource<Int, NoticeBody>() {

  // key 의 초기값은 null, load 함수 참고
  // 페이지를 갱신 할때 수행 되는 함수
  override fun getRefreshKey(state: PagingState<Int, NoticeBody>): Int? {
  ...
  override suspend fun load(params: LoadParams<Int>): LoadResult<Int, NoticeBody> {
    return try {
      val pageNumber = params.key ?: Constants.STARTING_PAGE_INDEX
      val response = apiService.getNoticeList()
      ...
      if (response.list.isNotEmpty()) {
        ...
        // 반환
        LoadResult.Page(
          data = response.list.map {noticeBody->
            noticeBody.toDomain()
          },
          prevKey = prevKey,
          nextKey = nextKey
